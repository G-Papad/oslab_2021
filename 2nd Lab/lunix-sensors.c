...
int lunix_sensor_init(struct lunix_sensor_struct *s)
{
	...
	for (i = 0; i < N_LUNIX_MSR; i++) {
		p = get_zeroed_page(GFP_KERNEL); //return a pointer to a new page and fills it with zeros.
		if (!p) {
			ret = -ENOMEM;
			goto out;
		}
		s->msr_data[i] = (struct lunix_msr_data_struct *)p; 
		//χωρίζει την σελίδα σε κομμάτια μεγέθους size_of(struct lunix_msr_data_struct)
		//βσικά στην αρχή της σελίδας αποθηκεύει μια δομή τύπου lunix_msr_data_struct
	...
	uint16_t batt, uint16_t temp, uint16_t light)
{
	 ...
	 */
	s->msr_data[BATT]->values[0] = batt; //to values exei tipo uint32_t alla edo mas dinoun dedomena uint16_t
